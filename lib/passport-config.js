...
passport.deserializeUser(async (req, id, done) => {
  const user = await User.findById(id)
    .populate({
      path: 'createdEvents participatingEvents',
      populate: {
...
passport.use(new LocalStrategy(
  ...
  async (req, email, password, done) => {
    ...
    console.log(regex);
    // TODO: Regexp insensitive matching or use .collation({strength: 1}) ?
    // Find user by email, (address is nested) and make sure to get the password
    // PW doesn't get sent normally, on purpose. Set in our model
    const user = await User.findOne({ 'email.address': { $regex: regex } }).select('+password');
