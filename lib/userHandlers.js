...
export const createUser = async (req, res) => {
  ...
  // TODO: I think this is suboptimal if the app gets used a lot. For our size, probably no big deal. But may need to adjust for a lowercase index?
  // Regexp version of case insensitve search in passportConfig. Don't know mongo well enough yet to know which is better.
  const alreadyExists = await User.find({ $or: [{ username }, { email: { address: email.address } }] }).collation({ locale: 'en_US', strength: 1 });
  if(alreadyExists.length > 0) {
    return res.status(422).json({ success: false, message: 'Username or email are already registered.' });
  }
  ...
  res.status(500).json({ success: false, message: 'Uncaught Error at Create User' });
};